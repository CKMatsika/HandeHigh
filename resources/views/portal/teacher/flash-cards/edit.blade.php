...
<div class="flex items-center justify-between mb-4">
    <div>
        <h1 class="text-lg font-semibold text-slate-50">Edit: {{ $set->title }}</h1>
        <p class="text-xs text-slate-400 mt-1">Update flash card terms and definitions</p>
    </div>
    <a href="{{ route('teacher.flash-cards.show', $set) }}" class="inline-flex items-center rounded-full border border-slate-700 bg-slate-900/80 px-4 py-1.5 text-xs font-medium text-slate-200 hover:bg-slate-800 transition">
        Back
    </a>
</div>

<div class="rounded-2xl border border-slate-800 bg-slate-900/80 p-4">
    <form method="POST" action="{{ route('teacher.flash-cards.update', $set) }}" id="flashCardForm">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div>
                ...
                <input type="text" name="title" value="{{ $set->title }}" required class="w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-xs text-slate-100 focus:border-indigo-500 focus:outline-none">
            </div>
            <div>
                ...
                <select name="subject_id" class="w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-xs text-slate-100 focus:border-indigo-500 focus:outline-none">
                    ...
                    @foreach($subjects as $subject)
                        <option value="{{ $subject->id }}" {{ $set->subject_id == $subject->id ? 'selected' : '' }}>{{ $subject->name }}</option>
                    @endforeach
                </select>
            </div>
            <div>
                <label class="block text-xs font-medium text-slate-300 mb-1">Class</label>
                <select name="school_class_id" class="w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-xs text-slate-100 focus:border-indigo-500 focus:outline-none">
                    ...
                    @foreach($classes as $class)
                        <option value="{{ $class->id }}" {{ $set->school_class_id == $class->id ? 'selected' : '' }}>{{ $class->name }}</option>
                    @endforeach
                </select>
            </div>
            <div>
                <label class="block text-xs font-medium text-slate-300 mb-1">Description</label>
                <input type="text" name="description" value="{{ $set->description }}" class="w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-xs text-slate-100 focus:border-indigo-500 focus:outline-none">
            </div>
        </div>
            ...
            @foreach($set->items as $i => $item)
                <div class="card-item rounded-xl border border-slate-700 bg-slate-950/60 p-3">
                    <div class="flex gap-2 items-start">
                        <div class="flex-1">
                            <label class="block text-[11px] font-medium text-slate-400 mb-1">Front</label>
                            <input type="text" name="items[{{ $i }}][front_text]" value="{{ $item->front_text }}" required class="w-full rounded-lg border border-slate-700 bg-slate-900 px-3 py-2 text-xs text-slate-100 focus:border-indigo-500 focus:outline-none">
                        </div>
                        <div class="flex-1">
                            <label class="block text-[11px] font-medium text-slate-400 mb-1">Back</label>
                            <input type="text" name="items[{{ $i }}][back_text]" value="{{ $item->back_text }}" required class="w-full rounded-lg border border-slate-700 bg-slate-900 px-3 py-2 text-xs text-slate-100 focus:border-indigo-500 focus:outline-none">
                        </div>
                        <div class="w-28">
                            <label class="block text-[11px] font-medium text-slate-400 mb-1">Hint</label>
                            <input type="text" name="items[{{ $i }}][hint]" value="{{ $item->hint }}" class="w-full rounded-lg border border-slate-700 bg-slate-900 px-3 py-2 text-xs text-slate-100 focus:border-indigo-500 focus:outline-none">
                        </div>
