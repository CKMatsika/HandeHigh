...
@section('content')
    <div class="flex items-center justify-between mb-4">
        <div>
            <h1 class="text-lg font-semibold text-slate-50">Edit Payment Method</h1>
            <p class="text-xs text-slate-400 mt-1">Update {{ $paymentMethod->name }} payment method</p>
        </div>
        <a href="{{ route('admin.payment-methods.show', $paymentMethod) }}" class="rounded-full bg-slate-700 px-4 py-2 text-xs font-medium text-white hover:bg-slate-600 transition">Cancel</a>
    </div>
            ...
            <div class="space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" id="code" name="code" value="{{ old('code', $paymentMethod->code) }}" required
                               class="w-full px-3 py-2 bg-slate-800 border border-slate-700 rounded-lg text-sm text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                               placeholder="e.g., ecocash">
                        @error('code')
                            <p class="mt-1 text-xs text-red-400">{{ $message }}</p>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select id="type" name="type" required
                            ...
                            <option value="card" {{ old('type', $paymentMethod->type) == 'card' ? 'selected' : '' }}>Card Payment</option>
                            <option value="bank_transfer" {{ old('type', $paymentMethod->type) == 'bank_transfer' ? 'selected' : '' }}>Bank Transfer</option>
                            <option value="online" {{ old('type', $paymentMethod->type) == 'online' ? 'selected' : '' }}>Online Payment</option>
                        </select>
                        @error('type')
                            <p class="mt-1 text-xs text-red-400">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <input type="text" id="provider" name="provider" value="{{ old('provider', $paymentMethod->provider) }}" required
                               class="w-full px-3 py-2 bg-slate-800 border border-slate-700 rounded-lg text-sm text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select id="currency" name="currency" required
                                class="w-full px-3 py-2 bg-slate-800 border border-slate-700 rounded-lg text-sm text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                            <option value="USD" {{ old('currency', $paymentMethod->currency) == 'USD' ? 'selected' : '' }}>USD - US Dollar</option>
                            <option value="ZWL" {{ old('currency', $paymentMethod->currency) == 'ZWL' ? 'selected' : '' }}>ZWL - Zimbabwe Dollar</option>
                            <option value="EUR" {{ old('currency', $paymentMethod->currency) == 'EUR' ? 'selected' : '' }}>EUR - Euro</option>
                            <option value="GBP" {{ old('currency', $paymentMethod->currency) == 'GBP' ? 'selected' : '' }}>GBP - British Pound</option>
                        </select>
                        @error('currency')
                            <p class="mt-1 text-xs text-red-400">{{ $message }}</p>
                        @enderror
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="3"
                              class="w-full px-3 py-2 bg-slate-800 border border-slate-700 rounded-lg text-sm text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                              placeholder="Describe this payment method...">{{ old('description', $paymentMethod->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-xs text-red-400">{{ $message }}</p>
                    @enderror
                </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="number" id="transaction_fee_percentage" name="transaction_fee_percentage" 
                                   value="{{ old('transaction_fee_percentage', $paymentMethod->transaction_fee_percentage) }}" required min="0" max="100" step="0.01"
                                   class="w-full px-3 py-2 bg-slate-800 border border-slate-700 rounded-lg text-sm text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                                   placeholder="0.00">
                        ...
                        <div>
                            ...
                            <input type="number" id="fixed_transaction_fee" name="fixed_transaction_fee" 
                                   value="{{ old('fixed_transaction_fee', $paymentMethod->fixed_transaction_fee) }}" required min="0" step="0.01"
                                   class="w-full px-3 py-2 bg-slate-800 border border-slate-700 rounded-lg text-sm text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                                   placeholder="0.00">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="number" id="minimum_amount" name="minimum_amount" 
                                   value="{{ old('minimum_amount', $paymentMethod->minimum_amount) }}" required min="0" step="0.01"
                                   class="w-full px-3 py-2 bg-slate-800 border border-slate-700 rounded-lg text-sm text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                                   placeholder="0.00">
                        ...
                        <div>
                            ...
                            <input type="number" id="maximum_amount" name="maximum_amount" 
                                   value="{{ old('maximum_amount', $paymentMethod->maximum_amount) }}" min="0" step="0.01"
                                   class="w-full px-3 py-2 bg-slate-800 border border-slate-700 rounded-lg text-sm text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                ...
                <div class="flex justify-end space-x-2 pt-4">
                    <a href="{{ route('admin.payment-methods.show', $paymentMethod) }}" class="rounded-full bg-slate-700 px-4 py-2 text-xs font-medium text-white hover:bg-slate-600 transition">Cancel</a>
                    <button type="submit" class="rounded-full bg-indigo-500 px-4 py-2 text-xs font-medium text-white hover:bg-indigo-600 transition">Update Payment Method</button>
                </div>
            </div>
        </form>
    </div>
