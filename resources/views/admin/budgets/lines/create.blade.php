...
@section('content')
    <div class="flex items-center justify-between mb-4">
        <div>
            <h1 class="text-lg font-semibold text-slate-50">Add Budget Line</h1>
            <p class="text-xs text-slate-400 mt-1">Add a new line item to {{ $budget->name }}</p>
        </div>
    ...
    <div class="bg-slate-900/50 border border-slate-800 rounded-xl p-6">
        <form action="{{ route('admin.budgets.lines.store', $budget) }}" method="POST">
            @csrf
            ...
            <div class="space-y-4">
                ...
                <div>
                    ...
                    <select id="cost_center_id" name="cost_center_id"
                        ...
                        @foreach($departments as $department)
                            <option value="{{ $department->id }}" {{ old('cost_center_id') == $department->id ? 'selected' : '' }}>
                                {{ $department->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <select id="period" name="period"
                            ...
                            @for($i = 1; $i <= 12; $i++)
                                <option value="{{ $i }}" {{ old('period') == $i ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <input type="number" id="budgeted_amount" name="budgeted_amount" value="{{ old('budgeted_amount') }}" required min="0" step="0.01"
                               class="w-full px-3 py-2 bg-slate-800 border border-slate-700 rounded-lg text-sm text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                               placeholder="0.00">
                ...
                <div>
                    ...
                    <textarea id="notes" name="notes" rows="3"
                              class="w-full px-3 py-2 bg-slate-800 border border-slate-700 rounded-lg text-sm text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent"
                              placeholder="Add any notes about this budget line...">{{ old('notes') }}</textarea>
                    @error('notes')
                        <p class="mt-1 text-xs text-red-400">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex justify-end space-x-2 pt-4">
                    <a href="{{ route('admin.budgets.show', $budget) }}" class="rounded-full bg-slate-700 px-4 py-2 text-xs font-medium text-white hover:bg-slate-600 transition">Cancel</a>
                    <button type="submit" class="rounded-full bg-indigo-500 px-4 py-2 text-xs font-medium text-white hover:bg-indigo-600 transition">Add Budget Line</button>
                </div>
            </div>
        </form>
    </div>
