...
@section('content')
    <div class="flex items-center justify-between mb-4">
        <div>
            <h1 class="text-lg font-semibold text-slate-50">Add Class</h1>
            <p class="text-xs text-slate-400 mt-1">Create a new class for a specific grade and academic year.</p>
        </div>
    ...
    <div class="rounded-2xl border border-slate-800 bg-slate-900/80 px-4 py-4">
        <form method="POST" action="{{ route('admin.classes.store') }}" class="space-y-4 text-xs text-slate-100">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input id="grade" name="grade" type="text" required class="w-full rounded-lg border border-slate-700 bg-slate-950/60 px-3 py-2 text-xs" value="{{ old('grade') }}">
                    @error('grade')
                        <p class="mt-1 text-[11px] text-red-400">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="flex justify-end">
                <button type="submit" class="inline-flex items-center rounded-full bg-indigo-500 px-4 py-1.5 text-xs font-medium text-white hover:bg-indigo-600 transition">Create Class</button>
            </div>
        </form>
    </div>
@endsection
