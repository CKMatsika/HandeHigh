...
@section('content')
    <div class="flex items-center justify-between mb-4">
        <div>
            <h1 class="text-lg font-semibold text-slate-50">Add Curriculum</h1>
            <p class="text-xs text-slate-400 mt-1">Assign a subject to a class with a teacher and schedule.</p>
        </div>
    ...
    <div class="rounded-2xl border border-slate-800 bg-slate-900/80 px-4 py-4">
        <form method="POST" action="{{ route('admin.curricula.store') }}" class="space-y-4 text-xs text-slate-100">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <select id="class_id" name="class_id" required class="w-full rounded-lg border border-slate-700 bg-slate-950/60 px-3 py-2 text-xs">
                        ...
                        @foreach($classes as $class)
                            <option value="{{ $class->id }}" {{ old('class_id') == $class->id ? 'selected' : '' }}>{{ $class->name }} ({{ $class->grade }})</option>
                        @endforeach
                    </select>
                    @error('class_id')
                ...
                <div>
                    ...
                    <input id="academic_year" name="academic_year" type="text" required class="w-full rounded-lg border border-slate-700 bg-slate-950/60 px-3 py-2 text-xs" value="{{ old('academic_year', now()->format('Y')) }}">
                    @error('academic_year')
                        <p class="mt-1 text-[11px] text-red-400">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input id="term" name="term" type="text" class="w-full rounded-lg border border-slate-700 bg-slate-950/60 px-3 py-2 text-xs" value="{{ old('term') }}">
                    @error('term')
                        <p class="mt-1 text-[11px] text-red-400">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <input id="weekly_periods" name="weekly_periods" type="number" min="1" max="20" required class="w-full rounded-lg border border-slate-700 bg-slate-950/60 px-3 py-2 text-xs" value="{{ old('weekly_periods', 1) }}">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="objectives" name="objectives" rows="3" class="w-full rounded-lg border border-slate-700 bg-slate-950/60 px-3 py-2 text-xs">{{ old('objectives') }}</textarea>
                    @error('objectives')
                        <p class="mt-1 text-[11px] text-red-400">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="materials" name="materials" rows="3" class="w-full rounded-lg border border-slate-700 bg-slate-950/60 px-3 py-2 text-xs">{{ old('materials') }}</textarea>
                    @error('materials')
                        <p class="mt-1 text-[11px] text-red-400">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="flex justify-end">
                <button type="submit" class="inline-flex items-center rounded-full bg-indigo-500 px-4 py-1.5 text-xs font-medium text-white hover:bg-indigo-600 transition">Create Curriculum</button>
            </div>
        </form>
    </div>
@endsection
