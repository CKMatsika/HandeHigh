...
@section('content')
    <div class="no-print flex items-center justify-between mb-4">
        ...
        <div class="flex items-center gap-2">
            ...
            <a href="{{ route('admin.reports.student-fee-collection') }}" class="text-xs text-slate-300 hover:text-white px-3 py-2">Refresh</a>
        </div>
    </div>

    <x-documents.school-header 
        :school="$school ?? null"
        title="Official Student Fee Billing & Collection Report"
        :subtitle="'Reporting Period: ' . $start . ' to ' . $end . ' · Grouped by: ' . ucfirst($groupBy) . ' · Total Collected: $' . number_format($totalCollected, 2) . ' (' . number_format($overallCollectionRate, 1) . '%)'"
        :date="now()"
    />
    ...
    <x-documents.school-footer 
        :school="$school ?? null"
        :show-banking="false"
        notice="Official student fee billing and collection report. Verified from student receivables account entries."
    />
@endsection
