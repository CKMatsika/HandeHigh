...
<div class="container-fluid">
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">
            <a href="{{ route('admin.sda.meetings.show', $meeting) }}" class="text-decoration-none text-gray-600">
    ...
    <div class="card shadow mb-4">
        <div class="card-body">
            <form action="{{ route('admin.sda.meetings.update', $meeting) }}" method="POST">
                @csrf
                @method('PUT')
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control @error('sda_committee_id') is-invalid @enderror" 
                                ...
                                @foreach($committees as $committee)
                                    <option value="{{ $committee->id }}" {{ old('sda_committee_id', $meeting->sda_committee_id) == $committee->id ? 'selected' : '' }}>
                                        {{ $committee->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select class="form-control @error('meeting_type') is-invalid @enderror" 
                                ...
                                <option value="regular" {{ old('meeting_type', $meeting->meeting_type) == 'regular' ? 'selected' : '' }}>Regular Meeting</option>
                                <option value="emergency" {{ old('meeting_type', $meeting->meeting_type) == 'emergency' ? 'selected' : '' }}>Emergency Meeting</option>
                                <option value="annual" {{ old('meeting_type', $meeting->meeting_type) == 'annual' ? 'selected' : '' }}>Annual General Meeting</option>
                                <option value="special" {{ old('meeting_type', $meeting->meeting_type) == 'special' ? 'selected' : '' }}>Special Meeting</option>
                            </select>
                            @error('meeting_type')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="datetime-local" class="form-control @error('meeting_date') is-invalid @enderror" 
                                   ...
                                   value="{{ old('meeting_date', $meeting->meeting_date->format('Y-m-d\TH:i')) }}" required>
                            @error('meeting_date')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control @error('venue') is-invalid @enderror" 
                                   id="venue" name="venue" value="{{ old('venue', $meeting->venue) }}" placeholder="e.g., Conference Room A">
                            @error('venue')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control @error('agenda') is-invalid @enderror" 
                              id="agenda" name="agenda" rows="5" placeholder="List the agenda items for this meeting...">{{ old('agenda', $meeting->agenda) }}</textarea>
                    @error('agenda')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="d-flex justify-content-between">
                    <a href="{{ route('admin.sda.meetings.show', $meeting) }}" class="btn btn-secondary">
