...
<div id="edit-header" class="flex items-center justify-between mb-4">
    <div class="flex items-center gap-3">
        <div>
            <h1 class="text-lg font-semibold text-slate-50">Edit Scheme of Work</h1>
            <p class="text-xs text-slate-400 mt-1">{{ $scheme->title }}</p>
        </div>
...
document.addEventListener('resources-selected', function(e) {
    ...
    if (textareas.length > 0) {
        ...
        lastTextarea.value = lastTextarea.value ? lastTextarea.value + '\n' + e.detail.resources : e.detail.resources;
    }
    ...
    <div class="rounded-2xl border border-slate-800 bg-slate-900/80 px-4 py-4 space-y-3">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="description" rows="2" class="w-full rounded-lg border border-slate-700 bg-slate-950/60 px-3 py-2 text-xs">{{ old('description', $scheme->description) }}</textarea>
            </div>
            <div>
                ...
                <select name="subject_id" required class="w-full rounded-lg border border-slate-700 bg-slate-950/60 px-3 py-2 text-xs">
                    ...
                    @foreach($subjects as $subject)
                        <option value="{{ $subject->id }}" {{ old('subject_id', $scheme->subject_id) == $subject->id ? 'selected' : '' }}>{{ $subject->name }}</option>
                    @endforeach
                </select>
            </div>
            <div>
            ...
            @foreach($scheme->items as $index => $item)
                <div class="session-item rounded-xl border border-slate-700 bg-slate-800/50 p-4 space-y-3">
                    <div class="flex items-center justify-between">
                        <span class="text-[11px] font-medium text-slate-400">Session {{ $index + 1 }}</span>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-3">
                        ...
                        <div>
                            ...
                            <select name="items[{{ $index }}][day_of_week]" required class="w-full rounded-lg border border-slate-700 bg-slate-950/60 px-3 py-2 text-xs">
                                @foreach(['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'] as $day)
                                    <option value="{{ $day }}" {{ old('items.'.$index.'.day_of_week', $item->day_of_week) === $day ? 'selected' : '' }}>{{ $day }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="md:col-span-2">
                    ...
                    <div>
                        ...
                        <textarea name="items[{{ $index }}][objectives]" rows="2" required
                            class="w-full rounded-lg border border-slate-700 bg-slate-950/60 px-3 py-2 text-xs">{{ old('items.'.$index.'.objectives', $item->objectives) }}</textarea>
                    </div>
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                        <div>
                            <label class="block text-[11px] font-medium text-slate-300 mb-1">Teaching Methods</label>
                            <input type="text" name="items[{{ $index }}][teaching_methods]" value="{{ old('items.'.$index.'.teaching_methods', $item->teaching_methods) }}"
                                class="w-full rounded-lg border border-slate-700 bg-slate-950/60 px-3 py-2 text-xs">
                        </div>
                        <div>
                            <label class="block text-[11px] font-medium text-slate-300 mb-1">Resources</label>
                            <input type="text" name="items[{{ $index }}][resources]" value="{{ old('items.'.$index.'.resources', $item->resources) }}"
                                class="w-full rounded-lg border border-slate-700 bg-slate-950/60 px-3 py-2 text-xs">
                        </div>
                        <div>
                            <label class="block text-[11px] font-medium text-slate-300 mb-1">Assessment</label>
                            <input type="text" name="items[{{ $index }}][assessment]" value="{{ old('items.'.$index.'.assessment', $item->assessment) }}"
                                class="w-full rounded-lg border border-slate-700 bg-slate-950/60 px-3 py-2 text-xs">
                        </div>
                    </div>
                    <div>
...
@push('scripts')
<script>
let sessionIndex = {{ count($scheme->items) }};
    ...
    const html = `
        <div class="session-item rounded-xl border border-slate-700 bg-slate-800/50 p-4 space-y-3">
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-3">
                <div>
                    <label class="block text-[11px] font-medium text-slate-300 mb-1">Week *</label>
                    <input type="number" name="items[${sessionIndex}][week_number]" min="1" value="1" required class="w-full rounded-lg border border-slate-700 bg-slate-950/60 px-3 py-2 text-xs">
                </div>
                <div>
                    <label class="block text-[11px] font-medium text-slate-300 mb-1">Day *</label>
                    <select name="items[${sessionIndex}][day_of_week]" required class="w-full rounded-lg border border-slate-700 bg-slate-950/60 px-3 py-2 text-xs">${dayOptions}</select>
                </div>
                <div class="md:col-span-2">
                    <label class="block text-[11px] font-medium text-slate-300 mb-1">Topic *</label>
                    <input type="text" name="items[${sessionIndex}][topic]" required class="w-full rounded-lg border border-slate-700 bg-slate-950/60 px-3 py-2 text-xs">
                </div>
            </div>
            <div>
                <label class="block text-[11px] font-medium text-slate-300 mb-1">Sub-topic</label>
                <input type="text" name="items[${sessionIndex}][sub_topic]" class="w-full rounded-lg border border-slate-700 bg-slate-950/60 px-3 py-2 text-xs">
            </div>
            <div>
                <label class="block text-[11px] font-medium text-slate-300 mb-1">Learning Objectives *</label>
                <textarea name="items[${sessionIndex}][objectives]" rows="2" required class="w-full rounded-lg border border-slate-700 bg-slate-950/60 px-3 py-2 text-xs"></textarea>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                <div>
                    <label class="block text-[11px] font-medium text-slate-300 mb-1">Teaching Methods</label>
                    <input type="text" name="items[${sessionIndex}][teaching_methods]" class="w-full rounded-lg border border-slate-700 bg-slate-950/60 px-3 py-2 text-xs">
                </div>
                <div>
                    <label class="block text-[11px] font-medium text-slate-300 mb-1">Resources</label>
                    <input type="text" name="items[${sessionIndex}][resources]" class="w-full rounded-lg border border-slate-700 bg-slate-950/60 px-3 py-2 text-xs">
                </div>
                <div>
                    <label class="block text-[11px] font-medium text-slate-300 mb-1">Assessment</label>
                    <input type="text" name="items[${sessionIndex}][assessment]" class="w-full rounded-lg border border-slate-700 bg-slate-950/60 px-3 py-2 text-xs">
                </div>
            </div>
            <div>
                <label class="block text-[11px] font-medium text-slate-300 mb-1">Remarks</label>
                <input type="text" name="items[${sessionIndex}][remarks]" class="w-full rounded-lg border border-slate-700 bg-slate-950/60 px-3 py-2 text-xs">
            </div>
        </div>
    `;
