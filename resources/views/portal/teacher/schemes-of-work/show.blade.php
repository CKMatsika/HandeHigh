...
<div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
    <div>
        ...
        <p class="text-xs text-slate-400 mt-1">
            {{ $scheme->subject->name ?? '' }} ({{ $scheme->subject->code ?? '' }}) &bull; {{ $scheme->schoolClass->name ?? '' }} &bull; {{ $scheme->academic_year }} {{ $scheme->term }}
        </p>
    </div>
    <div class="flex flex-wrap items-center gap-2">
        ...
        @if(in_array($scheme->status, ['draft', 'rejected']))
            <a href="{{ route('teacher.schemes-of-work.edit', $scheme) }}" class="inline-flex items-center rounded-xl bg-indigo-600 px-4 py-2 text-xs font-semibold text-white hover:bg-indigo-500 shadow-md shadow-indigo-500/20 transition">
                Edit Scheme
            </a>
        @endif
        @if(in_array($scheme->status, ['draft', 'preview', 'rejected']))
            <form method="POST" action="{{ route('teacher.schemes-of-work.submit', $scheme) }}" class="inline">
                @csrf
                <button type="submit" class="inline-flex items-center rounded-xl bg-emerald-600 px-4 py-2 text-xs font-semibold text-white hover:bg-emerald-500 shadow-md shadow-emerald-500/20 transition">
                    Submit for Review
                </button>
            </form>
        @endif
...
<div class="space-y-6 text-xs text-slate-100">
    ...
    <div class="rounded-2xl border border-slate-800 bg-slate-900/90 backdrop-blur-xl p-6 shadow-xl space-y-4">
        ...
        <div class="flex flex-col sm:flex-row items-center justify-between border-b border-slate-800 pb-4 gap-2">
            ...
            <div class="text-right flex flex-col sm:items-end">
                ...
                @if($scheme->submitted_at)
                    <span class="text-[10px] text-slate-500">Submitted: {{ $scheme->submitted_at->format('d M Y H:i') }}</span>
                @endif
            </div>
        </div>

        <!-- General Topic & Aims Block -->
        ...
        @if($scheme->review_notes && $scheme->status === 'rejected')
            <div class="rounded-xl border border-rose-500/40 bg-rose-500/10 p-3 text-xs text-rose-100">
                <strong class="font-semibold text-rose-200">Rejection Feedback from Headmaster/HOD:</strong> {{ $scheme->review_notes }}
            </div>
        @endif
    </div>
