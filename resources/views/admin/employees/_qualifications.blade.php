<div class="space-y-6">
    ...
    <form id="addQualForm" method="POST" action="{{ route('admin.employees.qualifications.store', $employee) }}" class="hidden rounded-xl border border-slate-700 bg-slate-800/50 p-4 space-y-4">
        @csrf
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                <label class="block text-xs text-slate-400 mb-1">Qualification Name *</label>
                <input type="text" name="name" required class="w-full px-3 py-2 bg-slate-700 border border-slate-600 rounded-lg text-slate-50 text-xs">
            </div>
            <div>
                <label class="block text-xs text-slate-400 mb-1">Institution *</label>
                <input type="text" name="institution" required class="w-full px-3 py-2 bg-slate-700 border border-slate-600 rounded-lg text-slate-50 text-xs">
            </div>
            <div>
                <label class="block text-xs text-slate-400 mb-1">Grade / Class</label>
                <input type="text" name="grade" class="w-full px-3 py-2 bg-slate-700 border border-slate-600 rounded-lg text-slate-50 text-xs">
            </div>
                    ...
                    <div class="flex items-start justify-between">
                        <div>
                            ...
                            @if($qual->year_start || $qual->year_end)
                                <p class="text-[10px] text-slate-500 mt-1">{{ $qual->year_start ?? '?' }} – {{ $qual->year_end ?? 'Present' }}</p>
                            @endif
                        </div>
                        <div class="flex items-center gap-2">
                            ...
                            @endif
                            <form method="POST" action="{{ route('admin.employees.qualifications.destroy', [$employee, $qual]) }}" onsubmit="return confirm('Delete?')">
                                @csrf @method('DELETE')
